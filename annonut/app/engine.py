...
def process_docx(input_path: str, output_path: str, whitelist: list, entities_to_mask: list):
    ...
    for para in doc.paragraphs:
        if para.text: para.text = traiter_texte(para.text)
...
def process_text(input_path: str, output_path: str, whitelist: list, entities_to_mask: list):
    ...
    with open(input_path, 'r', encoding='utf-8') as f: texte = f.read()
    if not texte.strip():
        with open(output_path, 'w', encoding='utf-8') as f: f.write(texte)
        return 0
    ...
    with open(output_path, 'w', encoding='utf-8') as f: f.write(texte_modifie)
    return mots_masques_total